...
const San_Function = require('../functions');
...
module.exports = (app) => {
  ...
  app.get('/api/testapi', async (req,res,next) => {
    return res.json({success:true,msg:'api tested Successfully'});
  });
    ...
    app.post('/api/get_docs', async (req, res, next) => {
        ...
        // }
        // else{
        //   return res.json({ success: false, msg: 'Token Is Required'});
        // }
        // Doc.find(query)
        // .exec()
  ...
  app.get('/api/doc/signers/:id',async (req,res) => {
    const doc = await Doc.findById(req.params.id);
    let ids = [];
    var fn = function asyncMultiplyBy2(v) {
      return new Promise((resolve, reject) => {
        v.drag_data.map((drag) => {
          if (!ids.includes(drag.signer_id)) {
            resolve(ids.push(drag.signer_id));
          } else {
            reject('')
          }
        });
    ...
    Promise.all(doc.images.map(fn)).then(function (data) {
      ...
      let final_data = [];
      query['_id'] = { $in: ids };
      let datareturn = ids.map(async (id)=>{
        let que = await Que.findOne({ doc_id: doc._id, signer_id: id});
        let signer = await Signer.findById(id);
        if (que && signer) {
          return final_data.push({ name: signer.name, color: signer.color, status: que.status }); 
        }
      });
      Promise.all(datareturn).then(() => {
        res.json(final_data);
      });
    });
  });
    ...
    User.find({ permissions: 'basic'})
      ...
      .exec(function (err, users) {
        User.count({}).exec(function (err, count) {
          if (err) return next(err)
          res.json({ users: users, page: Math.round(count / perPage), total_pages: count });
        })
      });
  });
  ...
  app.delete('/api/users/:id', (req, res, next) => {
    User.findOneAndRemove({ _id: req.params.id })
      ...
      .then((signer) => {
        try {
          Doc.deleteMany({ 'user_id': req.params.id }, function (err) { });
          Signer.deleteMany({ 'added_by': req.params.id }, function (err) { });
        } catch (e) {
          console.log(e);
        }
        res.json(signer);
  ...
  app.put('/api/user/update/:id', async (req, res) => {
    ...
    var mailOptions = {
      ...
      html: '<div><b><font style="font-family:tahoma;font-size:8pt">Find Credentials below for login:<br/>-------------------<br/>Email:- ' + user.email + '<br/>Password:- ' + req.body.password +'</font></b></div>'
    };
  ...
  app.post('/api/contacts', async (req, res, next) => {
    ...
    Guest.find({})
      ...
      .exec(function (err, contacts) {
        Guest.count({}).exec(function (err, count) {
          if (err) return next(err)
          res.json({ contacts: contacts, page: Math.round(count / perPage), total_pages: count });
        })
      });
  });

  app.delete('/api/contacts/:id', (req, res, next) => {
    Guest.findOneAndRemove({ _id: req.params.id })
      .exec()
      .then((contact) => {
        res.json(contact);
      })
      ...
      //   query['added_by'] = ObjectId(userMatched._id);
        var perPage = req.body.pageLimit;
          ...
          Signer.find({})
            ...
            .sort({ created_at: 'desc' })
            .populate('added_by')
            .exec(function (err, signers) {
              Signer.count({}).exec(function (err, count) {
                if (err) return next(err)
                res.json({ signers: signers, page: Math.round(count / perPage), total_pages: count});
              })
            });
      // }else{
      //   res.json({success:false})
  ...
  app.post('/api/signerswithdoc', async (req,res,next) => {
    ...
    app.delete('/api/doc/:id', (req, res, next) => {
        ...
        .then((doc) => {
          var fs = require('fs');
          if ( typeof doc.file !== 'undefined' && doc.file && fs.existsSync(process.env.APP_DIR+'server/uploads/docs/'+doc.file)){
            fs.unlink(process.env.APP_DIR+'server/uploads/docs/'+doc.file);
          }
          if ( typeof doc.images !== 'undefined' && doc.images[0]){
            doc.images.forEach(el => {
              if(fs.existsSync(process.env.APP_DIR+'server/uploads/docs/'+el.name)){
                fs.unlink(process.env.APP_DIR+'server/uploads/docs/'+el.name);
              }
            });
            let strn = doc.images[0].name.replace("_cnvrt_1", "").split('.');
            let pdf_file = strn[0]+'.pdf';
            if(pdf_file && fs.existsSync(pdf_file)){
              fs.unlink(process.env.APP_DIR+'server/uploads/docs/'+pdf_file);
            }
  ...
  app.delete('/api/signer/:id', (req, res, next) => {
    Signer.findOneAndRemove({ _id: req.params.id })
      .exec()
      .then((signer) => {
        Que.deleteMany({ 'signer_id': req.params.id }, function (err) {});
        res.json(signer);
      })
      .catch((err) => next(err));
  });
  ...
  app.post('/api/addprice',async(req,res) => {
    try {
      const { title, price, trial_dur, content, plan_dur, _id, trial_type, plan_type } = req.body;
      if (_id) {
        var planObj = await Plan.findById(_id);
      } else {
        var planObj = new Plan();
      }
      planObj.title = title;
      planObj.price = price;
      planObj.trial_dur = trial_dur;
      planObj.trial_type = trial_type;
      planObj.plan_dur = plan_dur;
      planObj.plan_type = plan_type;
  ...
  app.get('/api/edit/price/:id',async(req,res) => {
  ...
  app.delete('/api/prices/:id', async (req, res) => {
    Plan.findOneAndRemove({ _id: req.params.id })
      .exec()
      .then((price) => {
        res.json(price);
      })
  ...
  app.post('/api/admin',async(req,res) => {
    ...
    if (!userMatched) {
      return res.status(404).json({ error: 'Unauthroized' });
    }
    return res.json({ user: userMatched, settings: settings});
  });

  app.put('/api/admin', async (req, res, next) => {
    const user = await jwt.verify(req.body.token, config.JWT_SECRET);
    let userMatched = await User.findById(user.sub);
    userMatched.firstname = req.body.firstname;
    userMatched.lastname = req.body.lastname;
    userMatched.email = req.body.email;
    userMatched.mobile = req.body.mobile;
    if (req.body.old_password && req.body.password) {
      const samePassword = await userMatched.comparePassword(req.body.old_password);
      if (!samePassword) {
        return res.status(404).json({ error: 'Incorrect Password' });
      } else if (samePassword && req.body.old_password === req.body.password) {
        return res.status(404).json({ error: 'old and new passwords are same!' });
      }else{
        userMatched.password = bcrypt.hashSync(req.body.password, 10);
      }
    }
    userMatched.mobilecountrycode = req.body.mobilecountrycode;
    userMatched.save();
